 ...
 *
 * Four steps, one lit at a time, on a rail that draws downward behind them and
 * then starts over. The restart is the point: steps 2-4 are a loop she lives
 ...
 *   `setTimeout` per step - four state changes a loop, no per-frame work.
 ...
 * and it still lands exactly on them. The rail segments live *inside* each row
 * and stretch across the list gap with a negative bottom margin, for the same
 * reason: no fixed pixel geometry to re-tune when the copy changes length.
 */
...
export function HowLisaRuns({
  ...
  const steps = buildSteps(topLabel);
  ...
  /* Under reduced motion the loop never runs, so it settles on the last step:
     ...
     instead of being stranded on step 1 with three greyed-out rows. */
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      ...
      last ? LAST_STEP_MS : STEP_MS
    );
    return () => clearTimeout(t);
  }, [active, inView, reduced, steps.length]);
          ...
          <li key={step.title} className="relative flex gap-3">
            ...
            {isActive && (
              <motion.span
                ...
                className="absolute -inset-x-2 -inset-y-1.5 rounded-2xl bg-primary/6 ring-1 ring-primary/15"
              />
            )}
            ...
            <div aria-hidden className="relative flex w-7 shrink-0 flex-col items-center">
              ...
              {/* The rail. `-mb-4` cancels the list gap so the segments join up
                  into one continuous line; the fill inside it draws downward as
                  the step it leaves behind completes. */}
              {!isLast && (
